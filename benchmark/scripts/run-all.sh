...
main() {
    ...
    log_info "  - Volcano Version: ${VOLCANO_VERSION}"
    log_info "  - Test Scale: ${NUM_JOBS} jobs x ${PODS_PER_JOB} pods = ${TOTAL_PODS} total pods"
    log_info "  - Fake Nodes: ${NUM_FAKE_NODES}"
    log_info ""
    
    # Step 1: Setup Kind cluster
    log_step "Step 1/6: Setting up Kind cluster"
    "${SCRIPT_DIR}/01-setup-cluster.sh"
    
    # Step 2: Install Volcano
    log_step "Step 2/6: Installing Volcano"
    "${SCRIPT_DIR}/02-install-volcano.sh"
    
    # Step 3: Setup KWOK
    log_step "Step 3/6: Setting up KWOK and fake nodes"
    "${SCRIPT_DIR}/03-setup-kwok.sh"
    
    # Step 4: Setup monitoring
    log_step "Step 4/6: Setting up monitoring stack"
    "${SCRIPT_DIR}/04-setup-monitoring.sh"
