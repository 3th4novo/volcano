...
assert_contains "${queries}" "[10m:30s]"
...
[[ "${queries}" != *"[5m:30s]"* ]] || fail "promql output should use 10m windows, not 5m statistical windows"
[[ ! "${queries}" =~ (^|[^i])rate\(node_cpu_seconds_total ]] || fail "promql output should use adapter-aligned irate CPU queries"
[[ "${queries}" != *"30m"* ]] || fail "promql output should use 10m windows, not 30m"
