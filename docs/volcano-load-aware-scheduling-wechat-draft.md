...
这套方案不是要求业务改模板，也不要求客户给每类任务维护画像。用户只需要接入监控指标，再按业务特征调整少量参数。

对运维来说，节点为什么降分也更容易解释：真实负载多少，影子负载多少，CPU/内存权重怎么配，风险系数有没有生效，都能拆开看。

对业务来说，最直接的收益是批量发布和重部署时更不容易把新负载堆到同一台节点上。调度结果不一定追求绝对平均，但会更接近节点真实压力。
...
看增强前后 Pod 是否还集中落到少数节点，最高 CPU/Mem 峰值是否下降，节点负载曲线是否更平滑。

[待补充：Pod 数量、节点规格、批量创建速率、增强前后节点分布、最高 CPU/Mem 峰值、热点持续时间。]

### 滚动重部署

模拟客户日常发布，观察重部署期间局部节点是否快速升温，增强后热点有没有被削弱。

[待补充：Deployment 副本数、滚动批次、maxSurge/maxUnavailable、增强前后节点负载曲线、业务恢复时间。]

### request 偏低场景

构造 request 明显低于真实消耗的 Pod，验证 `burst_ratio` 是否能减少低估。

[待补充：request/limit 配置、实际资源消耗、不同 `burst_ratio` 下节点分布和负载峰值。]

### BestEffort 场景

构造无 request/limit 的 BestEffort Pod，验证 `be_cpu` 和 `be_mem` 是否能让它们参与节点评分。

[待补充：BestEffort Pod 数量、默认估算值与调优值对比、OOM 复现条件、增强后结果。]

### Gang 调度回滚

构造部分 Pod 临时分配后整体调度失败的 PodGroup，验证 Deallocate 后影子负载是否准确回退。
