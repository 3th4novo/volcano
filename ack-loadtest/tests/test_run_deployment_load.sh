...
assert_contains "${manifest}" "HOLD_SECONDS=\"86400\""
...
assert_contains "${queries}" "pod=~\"ack-resource-consumer-.*\""
assert_not_contains "${queries}" "pod=~\".*ack.*\""
...
assert_contains "${skewed_manifest}" "HOLD_SECONDS=\"86400\""
...
assert_contains "${skewed_manifest}" "memory: 600Mi"
assert_contains "${skewed_manifest}" "cpu: 200m"
assert_contains "${skewed_manifest}" "cpu: 250m"
